...
public class MonitorOneStepConvertRateAnalyze {
    ...
    public static void main(String[] args) {
        ...
        // 1、构造Spark上下文
        SparkConf conf = new SparkConf().setAppName("MonitorOneStepConvertRateAnalyze");
        ...
        final Broadcast<String> roadFlowBroadcast = sparkContext.broadcast(roadFlow);
    ...
    private static JavaPairRDD<String, Row> getCar2RowRDD(JavaRDD<Row> car2RowRDD) {
        return car2RowRDD.mapToPair(new PairFunction<Row, String, Row>() {
            private static final long serialVersionUID = -1818823597317043906L;
            @Override
        ...
        JavaPairRDD<String, Row> car2RowRDD)
    {
        ...
        {
            private static final long serialVersionUID = 6535735248953566785L;
            ...
            public Iterable<Tuple2<String, Long>> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
                 ...
                 *
                 * 	roadFlowBuilder怎么拼起来的？  rows已经有顺序，直接遍历然后追加到roadFlowBuilder。
                ...
                //[0001,0002,0003,0004,0005]，遍历分割完成的数组进行拼接
                for (int i = 1; i <= split.length; i++) {
                    String tmpRoadFlow = "";
                    //第一次进来：,0001，第二次进来：,0001,0002
                    for (int j = 0; j < i; j++) {
                        tmpRoadFlow += ","+split[j];
                    }
    ...
    private static Map<String, Long> getRoadFlowCount(JavaPairRDD<String, Long> roadSplitRDD)
    {
        JavaPairRDD<String, Long> sumByKey = roadSplitRDD.reduceByKey(new Function2<Long, Long, Long>() {
            private static final long serialVersionUID = 2735382905832634113L;
